...
- [Development guidelines](#development-guidelines)
    - [General BitcoinEvolution Core](#general-bitcoinevolution-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Run with the `-testnet` option to run with "play bitcoinevolutions" on the test network, if you
...
BitcoinEvolution Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/bitcoinevolution-project/bitcoinevolution/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_bitcoinevolution
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_bitcoinevolution --log_level=test_suite
$ valgrind -v --leak-check=full src/bitcoinevolutiond -printtoconsole
```
...
# A coverage report will now be accessible at `./test_bitcoinevolution.coverage/index.html`.
```

**Sanitizers**

BitcoinEvolution Core can be compiled with various "sanitizers" enabled, which add
...
- ThreadSocketHandler : Sends/Receives data from peers on port 22.
...
However, in open source software such as BitcoinEvolution Core, where everyone uses
...
pay attention to for reviewers of BitcoinEvolution Core code.

General BitcoinEvolution Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: BitcoinEvolution Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
Some of these are maintained by active developers of BitcoinEvolution Core, in which case changes should probably go
directly upstream without being PRed directly against the project.  They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project.  Changes to these should also
be sent upstream but bugfixes may also be prudent to PR against BitcoinEvolution Core so that they can be integrated
...
file descriptors it will cause problems with BitcoinEvolution's `select()` loop, because
...
```bash
$ lsof -p $(pidof bitcoinevolutiond) |\
...
would be to revert the upstream fix before applying the updates to BitcoinEvolution's
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:bitcoinevolution-project/bitcoinevolution.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `bitcoinevolution-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `bitcoinevolution-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of BitcoinEvolution Core, the wallet was always
